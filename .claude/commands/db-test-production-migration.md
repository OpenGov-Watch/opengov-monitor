...
1. **Download production database**
   ...
   # Download to local
   gcloud compute scp web-server:/tmp/polkadot-prod.db data/local/polkadot-prod.db \
...
2. **Check current version**
   ...
   sqlite3 data/local/polkadot-prod.db "SELECT version, name, applied_at FROM schema_migrations ORDER BY version"
   ```
...
3. **Run migrations against production copy**
   ```bash
   # Windows
   cd backend && .venv\Scripts\python.exe migrations\migration_runner.py --db ..\data\local\polkadot-prod.db

   # Unix
   cd backend && .venv/bin/python migrations/migration_runner.py --db ../data/local/polkadot-prod.db
   ```
...
4. **Verify results**
   ...
   sqlite3 data/local/polkadot-prod.db ".schema TableName"
   ```
...
5. **Test with application (optional)**
   ...
   DATABASE_PATH=data/local/polkadot-prod.db pnpm api:dev
   ```

## Notes

- Local `data/local/polkadot.db` remains untouched
