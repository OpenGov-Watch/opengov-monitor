...
class PriceService:
    ...
    This service provides price information for various cryptocurrencies from multiple
    sources (CoinGecko, Yahoo Finance). It handles:
    - Fetching current prices
    - Loading historical prices from cache
    - Converting between different assets
    - Caching price data for performance
    
    Configuration:
    - Prices are cached in a JSON file (prices.json)
    - Supports multiple price sources with fallback
    - Automatic retry on failed requests
    - Configurable timeout and retry settings via HttpClient
    
    Usage:
        price_service = PriceService()
        price = price_service.get_price(AssetKind.DOT)  # Get current DOT price
        converted = price_service.convert(100, AssetKind.DOT, AssetKind.USD)  # Convert DOT to USD
    """
    ...
    def __init__(self, network_info: NetworkInfo):
        ...
        Args:
            network_info: NetworkInfo instance for the target network
            
        Sets up the HTTP client and loads cached prices from disk.
        """
    ...
    def load_prices(self) -> None:
        ...
        3. Updates the internal price cache
        
        Raises:
            ValueError: If price fetching fails
        """
        # historic prices
