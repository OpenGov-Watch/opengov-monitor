...
def main():
    try:
        ...
        if spreadsheet_id is None:
            logger.warning("OPENGOV_MONITOR_SPREADSHEET_ID environment variable not set. Defaulting to test spreadsheet.")
            spreadsheet_id = default_spreadsheet_id
        assert spreadsheet_id is not None, "Please set the OPENGOV_MONITOR_SPREADSHEET_ID environment variable"

        logger.warning(f"GM https://docs.google.com/spreadsheets/d/{spreadsheet_id}/edit#gid=0")

        credentials_string = os.environ.get('OPENGOV_MONITOR_CREDENTIALS')
        if credentials_string is None:
            logger.warning("OPENGOV_MONITOR_CREDENTIALS environment variable not set. Trying to read from file.")
